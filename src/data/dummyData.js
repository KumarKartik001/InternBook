...
// // All intern IDs for assignment
// const internIds = DUMMY_USERS.filter(u => u.role === "intern").map(u => u.id);

// export const DUMMY_TASKS = Array.from({ length: 100 }).map((_, i) => {
//   const internId = internIds[i % internIds.length];
//   const mentorId = DUMMY_USERS.find(u => u.id === internId)?.mentorId;
...
//       "Security Patch",
//     ][i % 10]}`,
//     description: `Detailed description for Task ${i + 1}. This task involves ${["designing UI components", "writing backend logic", "optimizing queries", "writing test cases", "adding features"][i % 5]
//       } and collaborating with the mentor for approvals.`,
//     mentorId,
//     assignedTo: internId,
...
//     createdAt: `2025-${String((i % 12) + 1).padStart(2, "0")}-${String(((i + 5) % 28) + 1).padStart(2, "0")}`,
//   };
// });
